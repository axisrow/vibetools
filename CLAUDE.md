...
The golden rule: **`data/tools.yml` contains only human-curated data** — `name`, `url`, `category`, `description{en,ru}`. Everything else (stars, forks, `createdAt`, rank, growth, `new` flag) is **generated automatically** from the GitHub API and cached in `data/*.json`. Never add manual status flags to `tools.yml` — there is intentionally no `verified` field (it doesn't exist on star-history.com, so it doesn't exist here); `new` and `rank` are computed, not stored.
...
**`scripts/generate_readme.py`** owns the **taxonomy**: `CATEGORIES` (list of `(key, {title_en, title_ru, title_zh})`) is the single source of category order/names for README, site, and `fetch_candidates`. `CATEGORY_MAP` is exported for validation. `is_new(tool)` reads `created_at` (from repos-meta, enriched in `main`) for the `[new]` mark.

**`scripts/generate_site.py`** `build_data_json()` merges tools.yml + stars + repos-meta + history + trendshift-repos into one dict written to `docs/data.json` (loaded via `fetch` — same origin on Pages). Computes `rank` (by stars), `starsPerWeek` (7-day delta from history), `isNew` (createdAt ≤ NEW_DAYS). Trendshift-discovered repos (absent from tools.yml) are appended to the same `tools` payload with `trendshiftDiscovered: true` and `isNew: false` — **site-only** (README stays tools.yml-sourced; surfacing them in README is a separate decision). The site's top "Featured" strip is **not** a server-computed field: `renderFeatured()` in the template picks one random Trendshift tool client-side per page load from `DATA.tools.filter(toolHasTrendshift)`.
...
- **History ↔ starsPerWeek window contract.** `HISTORY_DAYS` (in update_stars, = 8) must be ≥ 8 so the 7-day `starsPerWeek` delta (`generate_site._stars_per_week` via `generate_readme._window_delta`) always has a today + 7-days-ago snapshot. `starsPerWeek` is **null** until stars-history.json holds both dated slices — it's empty until the second daily run (not a bug).
